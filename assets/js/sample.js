...
function resize(e) {
  ...
  var particleColumn = Math.floor(
    Math.min((canvas.width + 128) / PARTICLE_INTERVAL, PARTICLE_MAX_COLUMN)
  );
  ...
  for (var i = 0; i < PARTICLE_ROW; i++) {
    for (var j = 0; j < particleColumn; j++) {
      let x =
        PARTICLE_START_X +
        PARTICLE_INTERVAL * (j + (i % 2 == 0 ? 0 : 0.5)) +
        (Math.random() - 0.5) * 12;
      let y =
        PARTICLE_START_Y +
        PARTICLE_INTERVAL * 0.2 * i +
        i * i * 6 +
        (Math.random() - 0.5) * 2 * i;
      addParticle(x, y, i + 3);
      if (i != 0 && j != 0) {
...
function loop() {
  ...
  for (len = particles.length, i = 0; i < len; i++) {
    ...
    vx = 0;
    vy =
      (Math.sin(frame * 0.001 * TWO_PI + p.x * 0.01) * 4 +
    ...
    if (0 <= mousePos.y && mousePos.y <= h) {
      ...
      if (isIE) {
        ...
        dist = Math.hypot(mousePos.x - p.x - vx, (mousePos.y - p.y - vy) * 0.6); //判定円を縦長に
      let radius = 160.0;
      if (dist < radius) {
        vx +=
          (mousePos.x - p.x - vx) *
